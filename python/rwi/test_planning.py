...
from models import MAGICParamsNet, RLActorNet
...
def main():
    ...
    if not IS_SAC:
        params_model = MAGICParamsNet(PARTICLE_SIZES['PuckPush']).to(device).float()
        if model_path is not None:
            params_model.load_state_dict(torch.load(model_path))
    else:
    ...
    while True:
        ...
        with torch.no_grad():
            if not IS_SAC:
                params = params_model(torch.tensor(sgpc_belief, dtype=torch.float32, device=device).unsqueeze(0))[0].squeeze(0).cpu().numpy()
