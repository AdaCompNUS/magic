...
if __name__ == '__main__':
    ...
    params_model = MAGICParamsNet(PARTICLE_SIZE).to(device).double()
    ...
    while True:
        ...
        if len(data) > 2:
            ...
            with torch.no_grad():
                params = params_model(torch.tensor(state, dtype=torch.double, device=device).unsqueeze(0))[0].squeeze(0).cpu().numpy()
