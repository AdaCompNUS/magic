...
if __name__ == '__main__':
    ...
    params_model = MAGICParamsNet(PARTICLE_SIZE).float().to(device)
    params_model_optimizer = optim.Adam(params_model.parameters(), lr=LR)
    ...
    while True:
        ...
        # Respond params.
        with torch.no_grad():
            params = params_model(torch.tensor(state, dtype=torch.float, device=device).unsqueeze(0), PERTURBATION_STENGTH)[0].squeeze(0).float().cpu().numpy()
        ...
        params_model_optimizer.zero_grad()
        (-critic_loss).backward()
        torch.nn.utils.clip_grad_norm_(critic_model.parameters(), 1.0)
        critic_model_optimizer.step()

        # Update params model.
        (value, sd) = critic_model(sampled_states, params_model(sampled_states))
        params_loss = (-value).mean(dim=-1)
        critic_model_optimizer.zero_grad()
        params_model_optimizer.zero_grad()
        params_loss.backward()
        torch.nn.utils.clip_grad_norm_(params_model.parameters(), 1.0)
        params_model_optimizer.step()
        ...
        if previous_trajectory_result is not None:
            ...
            print('Step {}: Params Value = {}'.format(step, value.mean(dim=-1).detach().item()))
            print('Step {}: Params Value S.D. = {}'.format(step, sd.mean(dim=-1).detach().item()))
            print('Step {}: Elapsed = {} m'.format(step, (time.time() - start) / 60))

        # Save models.
        if step % SAVE_INTERVAL == 0:
            print('Saving models....')
            torch.save(params_model.state_dict(), save_path + 'params_model.pt.{:08d}'.format(step))
