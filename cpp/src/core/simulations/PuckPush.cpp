...
PuckPush::PuckPush() : step(0), _is_terminal(false) {
...
float PuckPush::BestReward() const {
  float distance = (PuckPush::GOAL - puck_position).norm() - (PuckPush::GOAL_RADIUS - PUCK_RADIUS);
  float max_distance_per_step = PuckPush::BOT_SPEED * DELTA;
  size_t steps = static_cast<size_t>(round(ceilf(distance / max_distance_per_step)));
  if (steps <= 1) {
    return PuckPush::GOAL_REWARD;
  } else {
    return (1 - powf(GAMMA, static_cast<float>(steps) - 1)) / (1 - static_cast<float>(steps)) * STEP_REWARD +
      powf(GAMMA, static_cast<float>(steps) - 1) * PuckPush::GOAL_REWARD;
  }
}
...
cv::Mat PuckPush::Render(const list_t<PuckPush>& belief_sims,
  ...
  // Draw board region.
  for (size_t i = 0; i < BOARD_REGION.size(); i++) {
    cv::line(frame,
        to_frame(BOARD_REGION[i]),
        to_frame(BOARD_REGION[(i + 1) % BOARD_REGION.size()]),
        cv::Scalar(0, 0, 0),
        5, cv::LINE_AA);
  }
