...
namespace RVO {
	...
	void RVOSimulator::doStep(size_t agent_index)
	{
		kdTree_->buildAgentTree();

    if (agent_index < std::numeric_limits<size_t>::max()) {
			agents_[agent_index]->computeNeighbors();
			agents_[agent_index]->computeNewVelocity();
			agents_[agent_index]->update();
    } else {
      for (size_t i = 0; i < agents_.size(); ++i) {
