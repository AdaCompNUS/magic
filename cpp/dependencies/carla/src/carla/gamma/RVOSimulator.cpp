...
#include "carla/gamma/Agent.h"
#include "carla/gamma/KdTree.h"
#include "carla/gamma/Obstacle.h"
...
namespace gamma {
	...
	std::vector<Vector2> RVOSimulator::bicycleMove (Vector2 cur_pos, Vector2 cur_heading, Vector2 pref_vel, float dt, float max_speed, float car_len, float max_tracking_angle_deg) {
		...
		float max_tracking_angle_rad = max_tracking_angle_deg * GAMMA_PI / 180.0f;
		...
		float turn = static_cast<float>(tanf (steer_) * distance / car_len);

		if ( static_cast<float>(fabs(turn)) < 0.0001f) { // use straight line model
			cur_heading = cur_heading.rotate (turn * 180.0f / GAMMA_PI);
			cur_pos += cur_heading * distance;
		}else { // use bicycle model
			float yaw =  atan2f(cur_heading.y(), cur_heading.x());
			float radius = distance / turn;

			float cx = static_cast<float>(cur_pos.x() -  sinf (yaw) * radius);
			float cy = static_cast<float>(cur_pos.y() +  cosf (yaw) * radius);
			yaw = static_cast<float>(fmod(yaw + turn, 2 * GAMMA_PI));
			float new_x = static_cast<float>(cx +  sinf (yaw) * radius);
			float new_y = static_cast<float>(cy -  cosf (yaw) * radius);
	...
	std::vector<Vector2> RVOSimulator::holonomicMove (Vector2 cur_pos, Vector2 cur_heading, Vector2 pref_vel, float dt, float max_speed, float car_len, float max_tracking_angle_deg) {
		...
		cur_heading = cur_heading.rotate (steer_ * 180.0f / GAMMA_PI);
		cur_pos += cur_heading * distance;
